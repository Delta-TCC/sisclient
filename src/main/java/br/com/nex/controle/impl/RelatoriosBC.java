...
public class RelatoriosBC{


			

	private final String PATH = "/WEB-INF/reports/";

	public boolean printReport(String format, Map params, String jasperName , List data, ServletContext servletContext) throws JRException, IOException {
		...
		try {
			...
			/* Importante: Se data � null, definir no relat�rio a seguinte propriedade:
			 * [When no data: All Sections, no detail.] ou
			 * Colocar um valor default em data, para sempre ter valores em detail.
			 */
			if (data == null){
			...
			InputStream fis = externalContext.getResourceAsStream(PATH + jasperName);
			JasperReport relatorio = (JasperReport) JRLoader.loadObject(fis);
            						
			if (format.equals("P")){
									
				JasperPrint jasperPrint = JasperFillManager.fillReport(relatorio, params, ds);
				responseStream = response.getOutputStream();
				
				JRExporter exp = new JRPdfExporter();
				exp.setParameter(JRExporterParameter.JASPER_PRINT, jasperPrint);
				exp.setParameter(JRExporterParameter.OUTPUT_STREAM, responseStream);				
				exp.exportReport();
	...
	public byte[] getPdfReportAsByteArray(String format, Map params, String jasperName, List data, ServletContext servletContext) throws JRException {
		...
		{						
			...
			JRBeanCollectionDataSource ds = new JRBeanCollectionDataSource(data);
			InputStream relatorio = externalContext.getResourceAsStream(PATH + jasperName);
	...
	public static byte[] mergeFilesInPages(List<byte[]> pdfFilesAsByteArray, Rectangle pageSize
			, float marginLeft,float marginRight, float marginTop,float marginBottom) throws DocumentException, IOException {
        ...
        //Para cada arquivo da lista, cria-se um PdfReader, respons�vel por ler o arquivo PDF e recuperar informa��es dele.
        for (byte[] pdfFile : pdfFilesAsByteArray) {
                
            PdfReader reader = new PdfReader(pdfFile);
            
            //Faz o processo de mesclagem por p�gina do arquivo, come�ando pela de n�mero 1.
            for (int i = 1; i <= reader.getNumberOfPages(); i++) {
                 ...
                 * cria-se uma nova p�gina no documento de destino.
                 */
